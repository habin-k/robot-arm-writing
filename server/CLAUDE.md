...
| 토픽 | 타입 | 발행자 | 구독자 |
|---|---|---|---|
| `/robot/status` | `String` | pub_sub.py | FastAPI (TODO) |
...
| 파일 | 내용 |
|---|---|
| `routers/robot.py` | `/robot/home`, `/robot/error-reset`, `/robot/jog` → ROS2 서비스 호출 |
| `websockets/robot.py` | `/robot/status` 토픽 구독 → WebSocket으로 전달 |
| `core/robot_state.py` | pub_sub.py 진행률 → `broadcast_progress()` 호출 |
