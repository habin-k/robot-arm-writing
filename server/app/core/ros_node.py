...
# task_manager가 발행하는 로봇 상태 문자열을 HMI 상태값으로 변환한다.
STATUS_MAP = {
    "WRITING": "writing",
    "HOMING":  "idle",
    "IDLE":    "done",
...
# 도화지 좌표(mm)를 로봇 BASE 좌표로 변환하기 위한 실측 기준값.
# A4 모서리 실측값에서 원점과 두 축 방향을 계산해 종이 기울기를 보정한다.
...
PAPER_ORIGIN_Z  = 98.0        # pen_down=False인 공이동 높이 계산용 기준 z
HOVER_HEIGHT    = 30.0
WRITE_RX, WRITE_RY, WRITE_RZ = 90.0, 180.0, 90.0


def paper_to_robot(px, py, pen_down):
    """도화지 좌표(path_generator 출력)를 로봇 BASE 좌표로 변환한다."""
